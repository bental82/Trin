...
export default function ReceptorTab({ tl, tN, tlAll, bridgeShow, setBridgeShow }) {
  const filtered = tl.filter(d => d.day <= 60 && d.day % 1 === 0);
  const merged = filtered.map((d) => {
    const i = tl.indexOf(d);
    const row = { ...d };
    Object.entries(tlAll).forEach(([k, bl]) => { row["bSV_" + k] = bl?.[i]?.sV ?? null; });
    return row;
  });
  const chartData = cleanZeroLine(merged, [...Object.keys(REC), "sV", ...Object.keys(tlAll).map(k => "bSV_" + k)]);
      ...
      <ResponsiveContainer width="100%" height={280} style={{ overflow: "visible" }}>
        <ComposedChart data={chartData} margin={{ top: 5, right: 6, left: -14, bottom: 5 }}>
          ...
          {Object.entries(STRATEGY_COLORS).map(([k, c]) =>
            bridgeShow[k] && <Line key={k} type="monotone" dataKey={"bSV_" + k} stroke={c} strokeWidth={1.5} dot={false} strokeDasharray="6 3" name={STRATEGY_LABELS[k] + " SERT"} connectNulls={false} />
          )}
