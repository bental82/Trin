...
export default function ReceptorTab({ tl, tN, tlBridge }) {
  const [range, setRange] = useState("day");
  const merged = tl.map((d, i) => ({ ...d, bridgeSV: tlBridge?.[i]?.sV ?? null }));
  const chartData = cleanZeroLine(filterByRange(merged, range), [...Object.keys(REC), "sV", "bridgeSV"]);
  return (
    <div>
      ...
      <ResponsiveContainer width="100%" height={280} style={{ overflow: "visible" }}>
        <ComposedChart data={chartData} margin={{ top: 5, right: 6, left: -14, bottom: 5 }}>
          ...
          <Line type="monotone" dataKey="bridgeSV" stroke="#0891b2" strokeWidth={2} dot={false} strokeDasharray="6 3" name="Bridge SERT" connectNulls={false} />
