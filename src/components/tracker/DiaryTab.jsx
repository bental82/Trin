...
import { pkCalc, computeAll, getDose, getTodayN, START } from "./pkEngine";
      ...
      <div style={{ marginTop: 16 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "auto 1fr auto auto auto", gap: "3px 8px", fontSize: 12, alignItems: "center" }}>
          ...
          {PILL_LOG.map(r => {
            ...
            dt.setDate(dt.getDate() + (r.day - 1));
            const ds = dt.toLocaleDateString("en-GB", { weekday: "short", month: "short", day: "numeric" });
            return (
              <React.Fragment key={r.day}>
                <div style={{ fontWeight: 600, color: "#0891b2" }}>D{r.day}</div>
                <div style={{ color: "#64748b" }}>{ds}</div>
